...
def compile_contract() -> None:
    cmd = ["solc", "--combined-json", "abi,bin", "ERC20.sol"]
    output_path = "compile.json"
    try:
        result = subprocess.run(cmd, capture_output=True, text=True, check=True)
        with open("compile.json", "w", encoding="utf-8") as f:
            f.write(result.stdout)
        print("Compiled to compile.json")
    except subprocess.CalledProcessError as e:
        print("Compilation failed. Run manually:")
        print("solc --combined-json abi,bin ERC20.sol > compile.json")
        if e.stdout:
            print("\n--- solc stdout ---\n", e.stdout)
        if e.stderr:
            print("\n--- solc stderr ---\n", e.stderr)
        ...
        if not os.path.exists(output_path) or os.path.getsize(output_path) == 0:
            raise FileNotFoundError("\n\nThe Solidity compiler solc was not found. Install solc, then run:\n"
...
class TestERC20Challenge(unittest.TestCase):
    ...
    def setUp(self) -> None:
        self.tester_provider: EthereumTesterProvider = EthereumTesterProvider()
        self.eth_tester: EthereumTester = self.tester_provider.ethereum_tester
    ...
    def test_totalSupply(self) -> None:
    ...
    def test_transferFromAndAllowance(self) -> None:
    ...
    def test_raw_logs(self) -> None:
        # Check that deposit appropriately emits Deposit event
        transfer_event_filter = self.contract.events.Transfer.create_filter(from_block='latest')
        approval_event_filter = self.contract.events.Approval.create_filter(from_block='latest')
    ...
    def test_failed_send_in_withdraw(self) -> None:
        external_code = """
            contract ERC20 {
